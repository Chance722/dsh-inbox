...
- **当前阶段**：**M0–M8 已验收**（阶段、验收标准与记录见 `docs/feature/dev-bus.md`）；此后按模块继续（面板中英双语等）。当前版本 `0.2.0`。
...
**只管"动过本仓库的回合"**：改代码 / 排查本仓库 / 维护文档 / 提交，这类回复才在**最末尾**声明**本轮**对 docs 知识文档的参考情况（docs/help/index.md 索引覆盖的文档）。
**只是"用插件"不算**——查收件箱、问用法、闲聊、问产品行为，一个字都不要写。
这条规则是给在本仓库里干活的 agent 的（dsh 会把工作区的 AGENTS.md 注入到**任何**在本仓库开的会话里，
见 `docs/help/dsh-plugin-platform.md`），用户日常用收件箱时看到它只会莫名其妙。

- "参考"= **本轮内实际读取过**文档（Read / WebFetch 等主动读取）；**编辑/修改文档不算阅读**——编辑发生在哪一轮，就在哪一轮声明
- 读了 → 列清单：文档名 + 反馈标记 + 用途一句话
- 在"动过本仓库"的前提下本轮没读任何 docs 文档（含跨轮延续：仅提交/复用上轮已读或已编辑的内容）→ 声明「本次未参考 docs 文档」（暴露文档盲区），**不要凭上轮记忆打 ✓**
