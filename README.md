...
- **Sync** — both directions as of 2026-09-20. **Push** writes each record twice: `inbox/sync/items/<id>.json` for the program, and `<id>.txt` **for you** — open it in the cloud drive and it is the record, with its text, its note and the attachments it points at. Attachment bytes land as `inbox/sync/attachments/<id>.<extension>` so images, videos and PDFs open as themselves. The settings form has 立即同步 = push then pull, and 全部重传 = ignore the "already pushed" cursor and send everything again. **Pull** still reads whatever you drop into the drop folder (at startup, and on demand). Pushes are incremental and attachment bytes are content-addressed, so the same bytes go up once. **Only credential bodies travel encrypted** (their `.txt` says so and nothing more); see the privacy rules below.
