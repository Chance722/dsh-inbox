...
| ✅ Says who judged it | Every record's category carries a coloured badge: **规则判定** (grey — the local rules), **模型判定** (violet — the capped model fallback), **手动判定** (blue — your own pick, the one nothing overwrites). Hovering explains it in one line |
