...
`{"msg":"未知运行时异常","code":500}` 是数据胶囊对**它不打算服务的东西**给的通用答复，不止签名一种来源：

- SigV2 请求得到它（上面第 3 条，实测）；
- **GET 一个目录占位对象**得到的也是它（2026-09-21 从插件的拉取里看到：`inbox：取对象失败：HTTP 500 — {"msg":"未知运行时异常","code":500}`，
  那次 GET 打的是列表里唯一一条不属于本机 `…/sync/` 树、名字恰好等于投放目录的条目）。

所以看到这句话时，先问"我请求的是不是它本来就不提供的东西"，别直接归到签名/凭证上；插件的对应修法见 `sync.md`
「投放目录不是一个文件」。
