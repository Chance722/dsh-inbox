...
#### M9 第十四步 — README 的安装/更新压成"命令 + 一句必要说明"（2026-09-21）

用户反馈："**README 写得太啰嗦了，这是面向用户的，直接写清楚安装命令是什么、更新命令是什么就好了。**"

- 把上一轮新增的「更新 / Update」**整节折进安装块**：一条命令配一条注释（安装 / 全新机器 / 更新），"重复跑 init 不会升级""刚发布时写确切版本"都落到命令行注释上，不再各占一段。
- 顺手合并的废话：平台说明并进第一句；"`init` 做三件事"的编号列表压成一段；"其它选项"的长解释只留旗标清单；"有两件事跟你的设置有关"并回同一段。
- 隐私一节里那条 profile 抓取身份也收短（去掉 UA 字面量与理由从句，细节仍指向 `docs/help/link-title-fetch.md`）。
- 数字：中英两份各 **-45/+21**、**-44/+21** 行（安装区从约 45 行降到 22 行）。
...
#### M9 第十七步 — 投放目录被当成"投放文件"读；发 0.2.8（2026-09-21）

用户贴回一条真机结果："**点击刷新我看有 500 报错**"，对应 `pull` 接口的返回：
`pulled: 0 / failed: 1 / skipped: 38 / skippedSync: 38 / skippedOlder: 0 / listed: 39`，
`reason` 是 `inbox：取对象失败：HTTP 500 — {"msg":"未知运行时异常","code":500}`。

**数字本身就把范围缩到一条**：`listed 39`、`skippedSync 38`、`skippedOlder 0` ⇒ 39 项里只有 **1 项**
不在任何 `…/sync/` 树下，而 `failed: 1` 里那个"文件名"正好是**投放目录名** `inbox`
⇒ 被当成投放文件去读的那一条，就是**目录本身**。S3 没有目录，云盘界面为了显示文件夹建的占位对象
key 以 `/` 结尾（`remote-sync-layout.md` 的表里早就记着它，只是当时以为它只影响观感）；
列举 `inbox/` 时它和普通对象一样被列出来，而这个网关对它不打算服务的东西一律回那句 500
（与 SigV2 那个 500 同一句话，见 `remote-gateway-compat.md`）。
**为什么一直是同一句**：拉取游标只推进到最后一次**成功**的条目（刻意的重试设计），
所以这一条永远比游标新 ⇒ 每次刷新重试一次、每次都失败，失败计数还盖住"到底有没有新东西"。

**修法**：`remote/pull.ts` 新增 `isFolder()`，在"要不要读"之前按**形状**跳过——key 以 `/` 结尾，
或路径就是这次列举的投放目录（网关把被列举的前缀本身当成一条返回的写法）；进 `skipped`，
并新增 `PullResult.skippedFolders`（面板不显示，供排查）。`…/sync/` 那半边的判定一动不动。
**不做"读失败了就算了"**：回 200 + 空 body 的网关会把占位对象当成一个空文本入库，
列表里从此多一条莫名其妙的记录——"目录不是文件"必须在"要不要读"之前判。

**验证**：真机数字复现——本地按同样三条形态搭的假服务，**关掉修法**时拿到的 reason 与用户贴的
**逐字一致**（`inbox：取对象失败：HTTP 500 — {"msg":"未知运行时异常","code":500}`）；
两条新测试（WebDAV 的 `inbox/sub/`、S3 的 `inbox/` 与 `inbox`）断言"**根本没发出那次 GET**"+ `failed: 0`，
关掉修法必红；`tsc --noEmit` 干净、**29 文件 309 条**全绿（+2）、`build` 通过。

**版本**：registry 上 latest 已是 **0.2.7**（`npm view` 实测，用户 `web` profile 的依赖也钉在 `0.2.7`），
所以这个修复**搭不了 0.2.7 的车** ⇒ `package.json` **0.2.7 → 0.2.8**；
README 中英两处"刚发布时写确切版本"的例子跟着改成 `@0.2.8`（`041fc5e` 定下的规矩：例子指正在发布的那个版本）；
AGENTS.md 那句过期的「当前版本」（还写着 `0.2.0`）一并更正并注明单一源。**发布由用户自己走。**

**没实测到的那一半**：没有对真桶重放这次 GET（凭据不在手上），所以"占位对象的确切 key 是 `inbox/`
还是不带斜杠的 `inbox`"是按上面的数字与失败名推断的；两种形状现在都跳，刷新后 `failed: 0` 即确认。
