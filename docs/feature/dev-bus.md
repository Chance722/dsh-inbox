...
**下一步**：① **先做右侧 dock 的 spike**（`ctx.sidebarRightTabs.register` + `sidebar.right.pane.tab`，仍未实测）；② 验证 Lucide 在 esbuild 产物里打包正常（普通依赖，会被打进 `lib/client.js`，不进 `dsh.client.external`）；③ 再把选定的列表模式折进真实代码（列表模式作为用户设置持久化）。
