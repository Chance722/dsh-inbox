...
**下一步（仍未做）**：① **右侧 dock 的 spike**（`ctx.sidebarRightTabs.register` + `sidebar.right.pane.tab`，仍未实测——这是 B 方案的命门）；② 灯箱放大与设置弹窗（原型已有，真实代码里还是旧的内联表单）；③ 列表模式作为**用户设置**持久化（现在是组件内 state，刷新就回默认单列）。
