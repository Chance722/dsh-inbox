...
**下一步**：① 等用户在 V1/V2/V3 里定（或组合）；② **先做右侧 dock 的 spike**（`ctx.sidebarRightTabs.register` + `sidebar.right.pane.tab`，这条我方仍未实测）；③ 用 Lucide 前先验证它在我们的 esbuild 产物里能正常打包（它是普通依赖、会被打进 `lib/client.js`，不进 `dsh.client.external`）。
