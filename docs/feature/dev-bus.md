...
| M4 | 对话工具与卡片 | 对话里取得到 | 检索/取回接口按约定返回（文本截断 1000 字、图片缩略图、链接卡、列表 10 条 + 还有 N 条）；截图留证 | 部分验收（工具全通；自定义卡片受 session 作用域所限，暂用内置卡片） |
...
`tool.call.toolview` 的 scope 是 **session**（证据：`dsh-client-ui-tool/lib/types/client/contract/slots.d.ts`）。我们在 root 半边的客户端插件里注册自定义卡片——没报错、产物里也有这段代码——但界面仍然用**内置通用卡片**渲染，所以缩略图目前显示为文本标记而不是图片。要修得先搞清"session 作用域的槽怎么注册"，留作下一轮的第一件事。

**遗留问题**

- 上面这条卡片问题（缩略图、链接卡、我们自己的排版都依赖它）。
- 工具目前不接受"未读之外"的更细过滤（如时间范围），也没有"标记已读"这类写操作——按产品决策，写操作留在面板里做。
