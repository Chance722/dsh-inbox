...
> **状态：pre-alpha，M0–M4 已完成。** 两个口能存、面板里能翻能管，**现在还能在对话里直接问**。自动分类和同步还在后面。一个已知缺口：对话里的工具结果目前是 dsh 内置卡片，不是我们自己的卡片（工具卡片槽是 session 作用域，详见开发总线）。进度和验收记录在[开发总线](docs/feature/dev-bus.md)。
...
| 🔒 两条硬拒绝 | 密钥类记录永不返回明文；图片字节永不进对话——只回一个标记，由界面在本机渲染 |
| ❌ 还没有 | 自动分类（M5）、同步（M6）、我们自己的对话卡片（见上面的缺口） |
