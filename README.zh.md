...
- **同步（单向）**——任何设备往远端（WebDAV 目录 / S3 桶）扔文件，仓库启动时**拉取**并分类；也只在启动时和点「刷新」时拉。**本机内容不会上传**：推送还没有做。

## 这个项目给自己定的隐私红线

- **账密加密落盘**（2026-09-20 起）：账密正文以**密文**存在 `items\*.json`（AES-256-GCM，密钥由你设的主密码 scrypt 派生）。**主密码和密钥都不落盘**——服务每次重启都要在「入库设置 → 账密加密」里重新解锁，密码忘了就解不开已有的密文（没有找回，这是设计）。没设主密码时，账密**不会被存进去**（宁可不存，也不写明文）。
