...
> **状态：pre-alpha，M0–M2 已完成。** 东西有两个口能进来（输入框的 `/inbox` 命令，或面板自己的捕获框），重复项会合并，仓库能存能列。分类、筛选、同步还在后面。进度和验收记录在[开发总线](docs/feature/dev-bus.md)。
...
| ✅ 存储、查询、合并 | 记录走 dsh 自己的存储栈持久化；同样的东西再存一次会并进原记录 |
| ✅ 最近存入列表 | 面板按时间倒序列出存过的东西，带类型/类目/状态/平台 |
| ❌ 还没有 | 分类、筛选、详情、回收站（M3–M5）、同步（M6） |
