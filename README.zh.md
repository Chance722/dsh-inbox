...
- **同步**——两个方向都在（2026-09-20 起）：**推送**把本机记录与附件写成 `inbox/sync/items/<id>.json`、`inbox/sync/attachments/<id>`（设置里有「立即同步」= 先推后拉，以及「全部重传」= 忽略"已推过"重新上传一遍，远端内容不对时用）；**拉取**照旧读你投放目录里的文件（启动时一次 + 手动）。推送是增量的（只传上次推送后改动过的），附件内容寻址、同一份只传一遍。**上云的密文只有账密正文**——见下面隐私红线。
