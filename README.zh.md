...
> **状态：pre-alpha，M0–M7 已完成，正在做 M8（打包与一键安装）。** 两个口能存、面板里能翻能管、对话里能直接问、存进来的东西已经分好类（规则优先，`deepseek-flash` 兜底），**账密加密落盘**，**和云盘双向同步**（推、拉、合并、删全在）。进度与验收记录在[开发总线](docs/feature/dev-bus.md)。
