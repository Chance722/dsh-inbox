...
| **面板** | 待看 / 类目 / 标签筛选 + 跨标题正文链接备注的搜索；两种列表密度；详情里能改名称、类目、备注、标签，能标待看，能删除与恢复 |
...
| **对话里取回** | 助手按关键词/类目/标签/待看/类型查（最多 10 条 + 还剩几条），按 id 打开一条（正文最多 1000 字、链接、备注、标签、附件信息） |
...
<!--
  截图槽位：把文件放进 docs/assets/（建议窗口宽度 ≥ 1200px，避免窄屏三栏塌成一栏），
  再把下面两行取消注释。真机截图比示意图好；README 顶部已经有一张封面图。

![面板：筛选、列表、详情](docs/assets/panel.png)
![设置：入库设置与账密加密](docs/assets/settings.png)
-->
...
- **图片**：分类时会把图片发给模型判断（手机拍的证件照比例与普通照片无异）；对话里只回 `[attachment:id]` 标记，字节不进对话。
