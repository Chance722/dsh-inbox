...
- **同步**——两个方向都在（2026-09-20 起）：**推送**把本机记录与附件写成 `inbox/sync/items/<id>.json`（给程序）+ `<id>.txt`（**给人看**：云盘里点开就是这条记录，正文、备注、附件指向都写在里面）与 `inbox/sync/attachments/<id>.<扩展名>`（图片/视频/PDF 原本就能打开）；设置里有「立即同步」= 先推后拉，以及「全部重传」= 忽略"已推过"重新上传一遍。**拉取**照旧读你投放目录里的文件（启动时一次 + 手动）。推送是增量的，附件内容寻址、同一份只传一遍。**上云的密文只有账密正文**（它的 `.txt` 只写"正文已加密"）——见下面隐私红线。
