...
> **状态：pre-alpha，M0 已完成。** 插件骨架能跑了：侧栏一个入口、一个整页面板、一个模型能调用的工具。仓库本体还没有。进度和验收记录在[开发总线](docs/feature/dev-bus.md)。
...
| ✅ 工具到得了模型 | `inbox_status` 已注册，对话里能调用 |
| ❌ 还没有 | 捕获、存储、分类、检索——M1–M6 才做 |
...
插件没有装进 `dsh` 本体，也没碰任何全局状态，所以删掉这两个目录就是彻底卸载。
