...
export const zh = {
  ...
  'detail.foreignWhy': '面板不内嵌别人的播放器，所以在浏览器里打开',
  'detail.foreignWhere': '那里才是大屏。',

  // Settings: credentials and remote.
  'settings.secrets': '账密加密',
  'settings.secretsWhy': '账密正文以密文写盘；主密码和密钥都不落盘，',
  'settings.secretsRestart': '重启后要重新解锁',
  'settings.secretsForgot': '。密码忘了就解不开，没有找回。',
  ...
  'settings.ingestWhy': '只做单向：远端往里扔，本机拉下来入库。密码走 dsh 的凭证库，不写进配置。',
  ...
  'settings.dirIsPrefix': '目录那一栏同时是「{dir}」的前缀（默认 {def}，会转成 {alt}）。',
  ...
  'settings.noSettingsService': '这个组合里没有设置服务，地址改不了',
  'settings.youUse': '你多半在用',
  'settings.headlessDev': '形态开发。',
  ...
  'manual.1.panel.body': '粘贴文字、链接、拖进图片文件，或点「选择文件」，然后「存入仓库」（快捷键也行）。',
  'manual.1.chat.label': '对话里',
  'manual.1.chat.body': '输入 /inbox 再跟文字或链接，图片直接附在输入框上；不发给模型。',
  ...
  'manual.3.icon.body': '各有图标；密钥 / 账密是钥匙，一眼能认出来。',
  'manual.3.who.label': '谁判的',
  'manual.3.who.body': '类目旁的标签分三种：规则判定 / 模型判定 / 手动判定（你选过的类目，规则和模型都不会覆盖）。',
  ...
  'manual.7.tools.label': '助手说不认识这个仓库',
  'manual.7.tools.body': '说明这个会话没带上收件箱插件。新开一个会话再问一次即可 —— 面板能用、助手看不到，这两件事是分开的。',
...
export const en: Dictionary = {
  ...
  'detail.foreignWhy': 'the panel does not embed other people’s players, so open it in the browser',
  'detail.foreignWhere': 'that is where the big screen is.',

  'settings.secrets': 'Credential encryption',
  'settings.secretsWhy': 'credential bodies are written to disk as ciphertext; neither the master password nor its key is ever written down,',
  'settings.secretsRestart': 'so every restart locks it again',
  'settings.secretsForgot': '. A forgotten password cannot be recovered.',
  ...
  'settings.ingestWhy': 'One way only: other devices drop files there, this machine pulls them in. The password lives in dsh’s credential store, not in the config.',
  ...
  'settings.dirIsPrefix': 'the directory is also the prefix of “{dir}” (default {def}, becomes {alt}).',
  ...
  'settings.noSettingsService': 'this composition has no settings service, so the address cannot be changed',
  'settings.youUse': 'you are most likely on the',
  'settings.headlessDev': 'shape.',
  ...
  'manual.title': 'Manual',
  'manual.subtitle': 'How to use it',
  'manual.lead': 'a three-minute read',
  'manual.1.title': '1. Getting things in',
  'manual.1.panel.label': 'Panel',
  'manual.1.panel.body': 'Paste text or a link, drop in images or files, or press “Choose a file” and then “Save to vault” (the keyboard shortcut works too).',
  'manual.1.chat.label': 'In the conversation',
  'manual.1.chat.body': 'type /inbox followed by text or a link; images ride along on the composer and are never sent to the model.',
  ...
  'manual.3.order.body': 'the name you typed → the fetched page title / the link / the text / the file name → the note (the last resort).',
  'manual.3.icon.label': 'Glyphs',
  'manual.3.icon.body': 'each kind has one; a credential is a key, recognisable at a glance.',
  'manual.3.who.label': 'Who judged it',
  'manual.3.who.body': 'the badge beside the category says 规则判定 / 模型判定 / 手动判定 — a category you chose is never overwritten by rules or the model.',
  ...
  'manual.7.tools.label': 'The assistant says it does not know this vault',
  'manual.7.tools.body': 'this session was not started with the inbox plugin. Open a new session and ask again — the panel working and the assistant seeing the tools are two different things.',
